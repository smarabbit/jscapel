...
IMAGE_NT_HEADERS* ReadHeaders(char *file_loc)
{
    ...
    if (DosHeader->e_magic != (0x5a4d)) {
        fprintf(stderr, "%s Error -- Not a valid PE file!\n",file_loc);
        return 0;
    }
...
gva_t* WL_Extract(char *file, gva_t* entries, gva_t *code_base){
	uint32_t numOfReloc, numOfExport;
	gva_t *relocTable, *exportTable, *mergeTable = 0, imageBase = 0, currentSize = 0;
	IMAGE_NT_HEADERS *PeHeader;

	//cleanup just in case
	WL_cleanUp();
	PeHeader = ReadHeaders(file);
	if(!PeHeader){
		WL_cleanUp();
		return 0;
	}

	relocTable = getRelocTable(PeHeader, &numOfReloc);
	exportTable = getExportTable(PeHeader, &numOfExport);
	imageBase = getImageBase(PeHeader);

	mergeTable = realloc(mergeTable, currentSize+sizeof(uint32_t));
	mergeTable[currentSize/sizeof(uint32_t)] = PeHeader->OptionalHeader.AddressOfEntryPoint+ imageBase;
	currentSize += sizeof(uint32_t);

	if(relocTable){
		uint32_t addSize = numOfReloc*sizeof(uint32_t);
		mergeTable = realloc(mergeTable, currentSize+addSize);
		memcpy(((BYTE *)mergeTable)+currentSize, relocTable, addSize);
	...
	if(exportTable){
		uint32_t addSize = numOfExport*sizeof(uint32_t);
		mergeTable = realloc(mergeTable, currentSize+addSize);
		memcpy(((BYTE *)mergeTable)+currentSize, exportTable, addSize);
		currentSize += addSize;
		free(exportTable);
		exportTable = 0;
	}
	(*code_base) = imageBase;
	(*entries) = currentSize/sizeof(uint32_t);
	return mergeTable;
}
...
int main(int argc, char **argv){
	...
	uint32_t *et = WL_Extract(argv[1], &et_num, &image_base);
	int i;
	if(et){
		for(i=0;i<et_num;i++){
			new_addr = et[i];
