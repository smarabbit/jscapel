...
struct StackFrame {
    ...
    ULONG col;
	ULONG position;
	ULONG size;// current statement size
	TCHAR* location;
    TCHAR* type;
    TCHAR * code;
	ULONG codesize;
...
private:
	std::wofstream jsTrace;
	std::wofstream codeTrace;
	typedef int (__cdecl *MYPROC)(const TCHAR *,int);
	MYPROC send_response;
	void sendResponse( tstring message);
	void sendResponse(StackFrame *frame);
	ULONG getStackFrameCount(IEnumDebugStackFrames *spDebugStackFrames );
	tstring getPropertyString(Property * pro);
    ...
    BOOL setBreakpointsForDocument(IDebugDocument *pDebugDocument, BREAKPOINT_STATE state);
